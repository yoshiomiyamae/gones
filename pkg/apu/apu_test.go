...
func TestChannelOutput(t *testing.T) {
	...
	// Writing $4003 resets Sequence to 0; duty 1 outputs 0 at position 0
	// and 1 at position 1. The real sequencer advances over many cycles
	// (timer ~256 × 2 stepPulse calls). For a unit test, jump directly to
	// the position where the duty waveform is high.
...
func TestAPUStep(t *testing.T) {
	...
	// One Step() ≈ one CPU cycle. The APU emits an audio sample every
	// ~40.58 CPU cycles (44100 Hz at 1.79 MHz), so iterate enough times to
	// guarantee at least one sample is queued.
	for i := 0; i < 100; i++ {
		apu.Step()
	}
	if len(apu.Output) == 0 {
		t.Error("Expected output buffer to have at least one sample after stepping")
	}
}
