...
func TestMapper4_MMC3(t *testing.T) {
	...
	t.Run("Mirroring_Control", func(t *testing.T) {
		...
		// MMC3 $A000 bit 0 uses *arrangement* terminology (NESdev MMC3 wiki:
		// "0: horizontal (A10); 1: vertical (A11)"), which inverts to PPU
		// *mirroring* terminology (0 = horizontal mirroring, 1 = vertical).
		mapper.WritePRG(0xA000, 0x00) // MMC3 horizontal arrangement -> PPU vertical mirroring
		if mapper.GetMirroringMode() != 1 {
			t.Errorf("Expected vertical mirroring (1), got %d", mapper.GetMirroringMode())
		}

		mapper.WritePRG(0xA000, 0x01) // MMC3 vertical arrangement -> PPU horizontal mirroring
